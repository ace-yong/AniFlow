...
从 [Releases](https://github.com/anomalyco/gamesky/releases) 下载最新版 `gamesky.exe`，放在任意目录，双击运行。

首次启动会提权（管理员权限），并在 exe 同级创建 `gamesky/` 数据目录：
...
```bash
git clone https://github.com/anomalyco/gamesky.git
cd gamesky
...
python -m PyInstaller gamesky.spec
# 输出: dist/gamesky.exe
```
